...
void ACROBOTIC_SSD1306::setTextXY(unsigned char row, unsigned char col)
{
    ...
    sendCommand(0x00 + ((m_font_width*col+OFFSET) & 0x0F));    //set column lower addr; add offset left
    sendCommand(0x10 + (((m_font_width*col+OFFSET)>>4) & 0x0F)); //set column higher addr; add offset left
}
...
void ACROBOTIC_SSD1306::clearDisplay()
{
  ...
  {    
    setTextXY(j,0);    
    {
      for(i=0;i<16+OFFSET*2;i++)  //clear all columns; driver is 132 wide !!!
      {
        putChar(' ');    
      }
    }
...
//       sendData(pgm_read_byte(&m_font[(ch-32)*m_font_width+m_font_offset+i])); 
     Wire.write(pgm_read_byte(&m_font[(ch-32)*m_font_width+m_font_offset+i]));
    }
     Wire.endTransmission();                    // stop I2C transmission
    return 1;
}
...
void ACROBOTIC_SSD1306::putLine(const char *string){
    unsigned char i=0;
    while(i<16 && string[i])
    {
        putChar(string[i]);     
